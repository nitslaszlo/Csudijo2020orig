...
FROM termekek
  INNER JOIN kategoriak
    ON termekek.kategoriaId = kategoriak.id
...
SELECT
  termekek.nev,
  SUM(rendelesek.mennyiseg) AS mennyiseg
FROM rendelesek
  INNER JOIN termekek
    ON termekek.id = rendelesek.termekId
WHERE etel = 1
GROUP BY termekek.nev
ORDER BY mennyiseg DESC LIMIT 3;
***
20. feladat
SELECT
  SUM(termekek.ar * rendelesek.mennyiseg) AS brutto,
